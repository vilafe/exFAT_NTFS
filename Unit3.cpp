...
void __fastcall Updater::UpdateVST()
{
	...
	while(true) {
		...
		PVirtualNode entry_node = Form1->VirtualStringTree1->AddChild(Form1->VirtualStringTree1->RootNode);
		s_node *p_node_data = (s_node*)Form1->VirtualStringTree1->GetNodeData(entry_node);
